...
public class InvoicesController {
	...
	public InvoicesController(InvoiceRepository invoices, CompanyRepository clients, BillingRecordRepository records) {
	...
	public ModelAndView showInvoices() {
		...
		// "invoices"
		mv.addObject("invoices", invoices.findAll());
		...
		mv.addObject("showTable", invoices.count() > 0);
		return mv;
	}
	...
	public ModelAndView chooseClient() {
		...
		mv.addObject("clients", clients.findAll());
		return mv;
	}
	...
	public ModelAndView createInvoice(@PathVariable Long clientId) {
		...
		Company client = clients.getOne(clientId);
		mv.addObject("records", records.findByClientAndLineItemIsNull(client));
	...
	public String createInvoice(Invoice invoice, @PathVariable Long clientId, long[] recordIds, Authentication auth) {
		...
		for (Long recordId : recordIds) {
			billingRecords.add(records.findOne(recordId));
		}
		...
		invoice.setCompany(clients.getOne(clientId));
